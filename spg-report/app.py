...
def show():
    ...
    orderReferer = request.args.get('orderReferer')
    ...
    if orderReferer == "spgLearningReport":
        pay_link = app.config['PARENT_PAY_LINK'] + '?sid=' + str(sid) + '&productType=' + productType
    else:
        pay_link = app.config['PAY_LINK']
        
    if data is None:
        # return render_template('error.html', reason='获取报告失败')
        return render_template('nodata.html', name = real_name, pay_link = pay_link, orderReferer = orderReferer)
